...
def basic_color(code):
	...
	def inner(text,rl= False):
		...
		c = code
		if rl:
			return"\001\033[%sm\002%s\001\033\002" %(c,text)
		else:
			return "\033[%sm%s\033[0m"%(c,text)
	return inner #where did text go
...
def color_func(func_name):
	...
	"""
	if str(func_name).isdigit(): #why use isdigit here
		return tem_color(int(func_name))
